...
from exceptions import NoGeneratorError, NoUpdateRequired
import datetime
...
class RDFUpdater(grok.Adapter):
    ...
    def updateRDF(self):
        ...
        if context.approvedFile:
            ...
            current = context.approvedFile.to_object.get_data()
        ...
        newFile = context[context.invokeFactory(
            ...
            title=u'RDF',
            description=u'Generated at %s by %s' % (datetime.datetime.utcnow().isoformat(), generatorPath),
            file=serialized,
        )]
